/*
 * =====================================================================================
 *        Sultan's Ultimate Windows Optimizer - V1.0 Stable Release
 * =====================================================================================
 * 
 * Developer:     0xSultan
...
namespace WinDebloatTools
{
    ...
    public class MainWindow : Form
    {
        ...
        {
            // Set Form Properties
            this.Text = "Sultan's Ultimate Windows Optimizer - v1.0";
            ...
            try
            {
                sysTrayIcon.Icon = Icon.ExtractAssociatedIcon(Application.ExecutablePath);
            }
            catch
            {
        ...
        private void InitializeLayout()
        {
            ...
            lblVersion.Text = "v1.0 by 0xSultan";
        ...
        private void CreateTweaksPanel()
        {
            ...
            pnlTweaksScroll.Location = new Point(20, 60);
            pnlTweaksScroll.Size = new Size(690, 480);
        ...
        private void ApplySelectedTweaks()
        {
            ...
            System.Threading.Tasks.Task.Factory.StartNew(() => {
                ...
                this.Invoke(new MethodInvoker(() => {
                    ...
                    MessageBox.Show(string.Format("Successfully applied {0} optimizations to the system! A restart is highly recommended.", successCount), 
                        "Tweaks Applied Successfully", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }));
            });
        }
        ...
        private List<BackupItem> GetBackups()
        {
            ...
            try
            {
                foreach (string file in Directory.GetFiles(baseDir, "debloat_backup_*.txt"))
                {
        ...
        {
            string timestamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
            return Path.Combine(AppDomain.CurrentDomain.BaseDirectory, string.Format("debloat_backup_{0}.txt", timestamp));
        }

        static void CreateBackupSilently()
        {
            string path = GetNewBackupFilePath();
            try { CreateBackupInternal(path, false); } catch {}
        }

        static void CreateBackupInternal(string path, bool verbose)
        {
            string backupDir = Path.GetDirectoryName(path);
            try
            {
                // Backup BCD Boot config
                string bcdBackupPath = Path.Combine(backupDir, "bcd_backup_" + Path.GetFileNameWithoutExtension(path));
                RunCommand("bcdedit", string.Format("/export \"{0}\"", bcdBackupPath));
                ...
                using (StreamWriter writer = new StreamWriter(path, false, System.Text.Encoding.UTF8))
                {
                    ...
                    foreach (string feature in optionalFeatures)
                    {
                        ...
                        writer.WriteLine(string.Format("SPECIAL|FeatureAgent|{0}|Special|Present|{1}", feature, enabled ? "True" : "False"));
                    }
                }
            }
            catch {}
            ...
            System.Threading.Tasks.Task.Factory.StartNew(() => {
                ...
                try
                {
                        ...
                        try
                        {
                            if (hive == "SPECIAL")
                            {
                                if (subKey == "MemoryAgent" && valueName == "MemoryCompression")
                                {
                                    ...
                                    SetMemoryCompression(originalEnabled);
                                }
                                ...
                                else if (subKey == "FeatureAgent")
                                {
                                    ...
                                    bool originalEnabled = (valueData == "True");
                                    SetFeatureState(featureName, originalEnabled);
                                }
                                restoreCount++;
                                continue;
                            }
                    ...
                    this.Invoke(new MethodInvoker(() => {
                        RestartExplorer();
                        SetControlsEnabled(true);
                        MessageBox.Show(string.Format("Restored {0} registry/service parameters successfully! Explorer was restarted.", restoreCount), 
                            "Restoration Completed", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        RefreshBackupsList();
                    }));
                }
                catch (Exception ex)
        ...
        static void RunCommand(string fileName, string arguments)
        {
            try
            {
                ...
                start.UseShellExecute = false;
                start.CreateNoWindow = true;
                using (Process process = Process.Start(start)) { process.WaitForExit(60000); }
            }
            catch {}
        }
        ...
        private void CleanSoftwareDistribution()
        {
            ...
            catch
            {
                try
                {
                    ...
                    start.UseShellExecute = false;
                    start.CreateNoWindow = true;
                    using (Process process = Process.Start(start)) { process.WaitForExit(10000); }
                }
                catch {}
            }
        ...
        private void OptimizeDrives()
        {
            try
            {
                ...
                start.UseShellExecute = false;
                start.CreateNoWindow = true;
                using (Process process = Process.Start(start)) { process.WaitForExit(90000); }
            }
            catch {}
        }
        ...
        static void PurgeStandbyList()
        {
            try
            {
                ...
                int result = Program.NtSetSystemInformation(80, pCommand, sizeof(int));
