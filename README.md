# Sultan's Ultimate Windows Optimizer - v1.1
...
### 4. 100% Symmetrical Backup & Restore
...
3. The compiled binary `UltimateTweakTool.exe` will be generated in the project directory, with an embedded administrator (UAC) manifest and version metadata.

> [!NOTE]
> `build.bat` compiles `WinDebloater.cs` together with `AssemblyInfo.cs` and embeds `app.manifest`. All three files ship in this repository — keep them next to `build.bat`.
...
2. Navigate through the sidebar tabs:
   ...
   * **Game Profiles**: Enter a game executable name (e.g., `cs2.exe`) and set a persistent CPU priority class and I/O priority level for it (re-applied every time the game launches, and removable any time from the same tab).
...
2. تنقل عبر التبويبات في واجهة الأداة الرسومية:
   ...
   * **Game Profiles (ملفات الألعاب)**: أضف أي ملف تشغيلي للعبة (مثل `cs2.exe`) لتطبيق أولوية CPU عالية وأولوية قراءة/كتابة قرص عالية بشكل ثابت (تُطبَّق في كل مرة تُشغّل فيها اللعبة، ويمكن إزالتها في أي وقت من نفس التبويب).
